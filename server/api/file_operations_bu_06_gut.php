...
switch ($action) {
    case 'list':
        ...
        if (!$hasYear && !$hasGallery) {
            ...
            echo json_encode(listFolderTree($uploadsDir));
            break;
        }
        // Bisheriges Verhalten: Dateien eines bestimmten Jahr/Gallery-Ordners
        ...
        $hiddenFiles = ['.htaccess', '.htaccess.allow', '.keep'];
        $files = array_values(array_filter(scandir($dir), function($f) use ($dir, $hiddenFiles) {
            return is_file("$dir/$f") && $f[0] !== '.' && !in_array($f, $hiddenFiles);
        }));
    ...
    case 'create_folder':
        ...
        $safePath = preg_replace('/[^a-zA-Z0-9_\/\-öäü@À-ÿ&\.\s]/u', '', $path); // Sicherheit: nur erlaubte Zeichen
    ...
    case 'upload':
        ...
        $safePath = preg_replace('/[^a-zA-Z0-9_\/-]/', '', $_POST['path']);
        $targetDir = rtrim($uploadsDir, '/') . '/' . ltrim($safePath, '/');
        if (strpos(realpath($targetDir), realpath($uploadsDir)) !== 0) {
            http_response_code(403);
            echo json_encode(['error' => 'Forbidden']);
            exit;
        }
    ...
    case 'upload_chunk':
        ...
        try {
            ...
            $tempDir = sys_get_temp_dir() . '/file_uploads/' . md5($fileName . $fileSize);
            if (!is_dir($tempDir)) {
            ...
            if ($chunkIndex === $totalChunks - 1) {
                ...
                for ($i = 0; $i < $totalChunks; $i++) {
                    ...
                    fclose($in);
                    unlink($chunkFile); // Delete the chunk after combining
                }
