...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (data && data.users && Array.isArray(data.users)) {
        ...
        // Fallback für den Fall, dass die API ein direktes Array zurückgibt
        setUsers(data);
      } else {
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`Sende Update-Anfrage für Benutzer ${editingUser.id}:`, updateData);
      
      // Versuche zuerst die [id]-Route
      ...
      // Wenn die [id]-Route nicht funktioniert, versuche die Haupt-Route
      if (response.status === 404) {
        console.log("[id]-Route nicht gefunden, versuche Haupt-Route");
        response = await fetch(`/api/users`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ ...updateData, id: editingUser.id }),
        });
      }
